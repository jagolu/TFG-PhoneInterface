...
export class DoFootballBetComponent implements OnInit{
   ...
   * 
   * @access private
   * @var {number} _selectedWinner
   */
  private _selectedWinner:number = 0;
  ...
  ngOnInit(){
    ...
    this.show1X2 = this.info.bet.typeBet.name.includes("WINNER");
    //Get the winrate of the bet
    this.win_rate = this.info.bet.typeBet.winRate+this.info.bet.typePay.winRate;
    ...
    this.min = this.info.bet.minBet;
    //The max of the bet
    this.max = this.info.bet.maxBet;
    // If it is a group bet, it calculate the jackpot
    this.jackpot = this.jackpotBet ? this.info.bet.usersJoined * this.min : 0;
    ...
    this.user_coins = this.jackpotBet ? this.info.userCoins-this.min : this.info.userCoins;
    //The max what user can bet (the min value of the maxBet and the actual user coins)
    this.max_user = Math.min(this.info.bet.maxBet, this.info.userCoins);
    //The id of the bet
    this._bet = this.info.bet.bet;
   ...
   * @access public
   */
  public setMessage(selectedIndex:number){
    this.radioButton(selectedIndex);
  ...
  private betReq(){
    this._betS.doFootballBet({
      ...
      "winner": !this.show1X2 ? null : this._selectedWinner,
    });
  }
  ...
  /**
   * Gives a correct functionality to the radio button
   * 
   * @access private
   * @param {number} selectedIndex The selected index of the
   * radio group
   */
  private radioButton(selectedIndex:number){
    (document.querySelector("#winner0") as HTMLIonRadioElement).checked = selectedIndex == 0;
    (document.querySelector("#winner1") as HTMLIonRadioElement).checked = selectedIndex == 1;
    (document.querySelector("#winner2") as HTMLIonRadioElement).checked = selectedIndex == 2;
    this._selectedWinner = selectedIndex;
  }
}